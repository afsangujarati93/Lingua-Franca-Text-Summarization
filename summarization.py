...
def main():
	...
	try:
		...
		if not (summary_case == 1 or summary_case == 2):
			...
			print("\n>>>>>>>>>>>>>>>>>>>>>>>>Important Notes<<<<<<<<<<<<<<<<<<<<<<<<<\n")
			print("Input file/s should be in >>'"+ input_dir +"'<<")
			print("Title file should have >>SAME NAME<< as input file, but EXTENSION should be >>title<<")
			print("Model Summary file should have >>SAME NAME<< as input file, but EXTENSION should be >>model<<")
			# print("Model Summary/ies should be in >>'"+ model_dir + "'<<")
		elif summary_case == 2:
			input_dir = "Final Input Data/"
			print("\n>>>>>>>>>>>>>>>>>>>>>>>>Important Notes<<<<<<<<<<<<<<<<<<<<<<<<<\n")
			print("Input file should be in >>'"+ input_dir +"'<<")
			print("Title file should have >>SAME NAME<< as input file, but EXTENSION should be >>title<<")			
		...
		for file in os.listdir(input_dir):			
			if file.endswith(".txt"):
				...
				lingua_franca_summary = summarization_process(input_file_name, input_data_txt, input_title_txt, sent_count)
				if show_summaries:
		...
		if summary_case == 1:
			...
			df_add = df_add/list_len
			df_add.insert(loc=0, column='Summarizers Name', value = summarizer_list)
...
def summarization_process(file_name, input_data_txt, input_title_txt, sent_count):
	method_name = inspect.stack()[0][3]
	try:
		...
		df_features["language_relevance"] = language_relevance(file_name, pp_returned_data.tokenized_word_sentences, pp_returned_data.processed_word_list)
...
def language_relevance(file_name, tokenized_word_sentences, language_relevant_words):
	method_name = inspect.stack()[0][3]
	try:
		process_logger.debug("in "+ method_name +" method")
		lang_rel_list = []
